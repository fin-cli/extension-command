Feature: List recently active FinPress plugins
  ...
  Scenario: Verify plugin installation, activation, deactivation and confirm listing recently active plugins list is correct
    ...
    When I run `fin plugin install site-secrets debug-bar p2-by-email --activate`
    ...
    When I run `fin plugin list --recently-active --field=name --format=json`
    ...
    When I run `fin plugin deactivate site-secrets debug-bar`
    Then STDOUT should contain:
      """
    ...
    When I run `fin plugin list --recently-active --field=name`
  ...
  Scenario: Use recently active plugin to activate plugins
    ...
    When I run `fin plugin install site-secrets debug-bar --activate`
    Then STDOUT should contain:
      """
      Plugin 'site-secrets' activated.
      """
    ...
    When I run `fin plugin activate $(fin plugin list --recently-active --field=name)`
    Then STDOUT should contain:
      """
      Plugin 'debug-bar' activated.
      """
  ...
  Scenario: For a MU site, verify plugin installation, activation, deactivation and confirm listing recently active plugins list is correct
    ...
    When I run `fin plugin install site-secrets debug-bar p2-by-email --activate-network`
    Then STDOUT should contain:
      """
    ...
    Then STDOUT should be:
      ...
      []
      """
    When I run `fin plugin deactivate site-secrets debug-bar --network`
    Then STDOUT should be:
      """
  ...
  Scenario: For a MU site, use recently active plugin to activate plugins
    ...
    When I run `fin plugin install site-secrets debug-bar --activate-network`
    Then STDOUT should contain:
      """
      Plugin 'site-secrets' network activated.
      """
    ...
    When I run `fin plugin activate $(fin plugin list --recently-active --field=name) --network`
    Then STDOUT should contain:
      """
      Plugin 'site-secrets' network activated.
      """
