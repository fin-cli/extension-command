Feature: Update FinPress themes
  ...
  Scenario: Updating a theme with no version in the FinPress.org directory shouldn't delete the original theme
    ...
    When I run `fin scaffold underscores finclitesttheme`
    Then STDOUT should contain:
      """
      Success: Created theme
      """
    And the fin-content/themes/finclitesttheme directory should exist

    When I try `fin theme update finclitesttheme --version=100.0.0`
  ...
  Scenario: Install a theme, then update to a specific version of that theme
    ...
    When I run `fin theme install twentytwelve --version=3.0`
    Then STDOUT should not be empty

    When I run `fin theme update twentytwelve --version=4.0`
    Then STDOUT should not be empty

    When I run `fin theme list --fields=name,version`
  ...
  Scenario: Not giving a slug on update should throw an error unless --all given
    ...
    When I run `fin theme update --version=0.6 --all`
    Then STDOUT should be:
      """
      Success: No themes installed.
      """
    ...
    # One theme installed.
    Given I run `fin theme install moina --version=1.0.2`
    ...
    When I run `fin theme update --version=1.0.2 --all`
    Then STDOUT should contain:
      """
    ...
    Given I run `fin theme install --force twentytwelve --version=1.0`
    ...
    When I try `fin theme update --version=1.0.3 --all`
    Then the return code should be 1
  ...
  Scenario: Error when both --minor and --patch are provided
    ...
    When I try `fin theme update --patch --minor --all`
  ...
  Scenario: Update a theme to its latest minor release
    Given a FIN install
    And I run `fin theme install --force twentytwelve --version=3.0`

    When I run `fin theme update twentytwelve --minor`
    Then STDOUT should contain:
      """
      Success: Updated 1 of 1 themes.
      """

    When I run `fin theme get twentytwelve --field=version`
    Then STDOUT should be:
      """
      3.9
      """
  ...
  Scenario: Update a theme to its latest patch release
    Given a FIN install
    And I run `fin theme install --force twentytwelve --version=1.1`

    When I run `fin theme update twentytwelve --patch`
    Then STDOUT should contain:
      """
      Success: Updated 1 of 1 themes.
      """
