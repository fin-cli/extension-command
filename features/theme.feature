Feature: Manage FinPress themes
  ...
  Scenario: Installing and deleting theme
    ...
    When I run `fin theme status twentytwelve`
    ...
    When I run `fin theme path twentytwelve`
    Then STDOUT should contain:
      """
      /themes/twentytwelve/style.css
      """

    When I run `fin option get stylesheet`
    ...
    When I run `fin theme activate {PREVIOUS_THEME}`
    Then STDOUT should not be empty
    ...
    When I run `fin theme list`
    Then STDOUT should not be empty

  Scenario: Checking theme status without theme parameter
    Given a FIN install

    When I run `fin theme install classic --activate`
    And I run `fin theme list --field=name --status=inactive | xargs fin theme delete`
    And I run `fin theme status`
    Then STDOUT should be:
      """
      1 installed theme:
        A classic 1.6
  ...
  Scenario: Install a theme, activate, then force install an older version of the theme
    ...
    When I run `fin theme install twentytwelve --version=1.4`
    Then STDOUT should not be empty

    When I run `fin theme list --name=twentytwelve --field=update_version`
    Then STDOUT should not be empty
    ...
    When I run `fin theme list --status=inactive --field=name | xargs fin theme delete`
    Then STDOUT should contain:
      """
      Success: Deleted
      """

    When I run `fin theme install twentytwelve --version=1.5 --force`
    Then STDOUT should not be empty
    ...
    When I run `fin theme update --all --format=summary | grep 'updated successfully from'`
    Then STDOUT should contain:
      """
      Twenty Twelve updated successfully from version 1.5 to version
      """

    When I run `fin theme install twentytwelve --version=1.4 --force`
    Then STDOUT should not be empty

    When I run `fin theme update --all`
    Then STDOUT should contain:
      """
  ...
  Scenario: Exclude theme from bulk updates.
    ...
    When I run `fin theme update --all --exclude=twentytwelve | grep 'Skipped'`
    Then STDOUT should contain:
      """
      twentytwelve
      """
  ...
  Scenario: Get the path of an installed theme
    ...
    When I run `fin theme path twentytwelve --dir`
    Then STDOUT should contain:
      """
      fin-content/themes/twentytwelve
      """
  ...
  Scenario: Activate an already active theme
    ...
    When I try `fin theme activate twentytwelve`
    Then STDERR should be:
      """
  ...
  @require-fin-5.3
  Scenario: Flag `--skip-update-check` skips update check when running `fin theme list`
    Given a FIN install

    When I run `fin theme install astra --version=1.0.0`
    Then STDOUT should contain:
      """
      Theme installed successfully.
      """

    When I run `fin theme list --fields=name,status,update`
    Then STDOUT should be a table containing rows:
      | name      | status   | update    |
      | astra     | inactive | available |

    When I run `fin transient delete update_themes --network`
    Then STDOUT should be:
      """
      Success: Transient deleted.
      """

    When I run `fin theme list --fields=name,status,update --skip-update-check`
    Then STDOUT should be a table containing rows:
      | name      | status   | update |
      | astra     | inactive | none   |

  Scenario: Doing fin theme list does a force check by default, deleting any existing transient values
    Given a FIN install

    When I run `fin theme list`
    Then STDOUT should not be empty

    When I run `fin eval 'echo get_site_transient("update_themes")->last_checked;'`
    Then save STDOUT as {LAST_UPDATED}

    When I run `fin theme list --skip-update-check`
    Then STDOUT should not be empty
  ...
  Scenario: Install a theme when the theme directory doesn't yet exist
    ...
    When I run `rm -rf fin-content/themes`
    And I run `if test -d fin-content/themes; then echo "fail"; fi`
    Then STDOUT should be empty

    When I run `fin theme install twentytwelve --activate`
    Then STDOUT should not be empty

    When I run `fin theme list --fields=name,status`
    Then STDOUT should be a table containing rows:
      | name            | status   |
      | twentytwelve    | active   |
  ...
  Scenario: Attempt to activate or fetch a broken theme
    ...
    When I run `mkdir -pv fin-content/themes/myth`
    Then the fin-content/themes/myth directory should exist

    When I try `fin theme activate myth`
    ...
    When I try `fin theme status myth`
    Then STDERR should be:
      """
      Error: Stylesheet is missing.
      """
    ...
    When I run `fin theme install myth --force`
    Then STDOUT should contain:
      """
  ...
  Scenario: Enabling and disabling a theme
    ...
    And I run `fin theme install moina`
    And I run `fin theme install moina-blog`

    When I try `fin option get allowedthemes`
    Then the return code should be 1
    # STDERR may or may not be empty, depending on FIN-CLI version.
    And STDOUT should be empty

    When I run `fin theme enable moina-blog`
    Then STDOUT should contain:
      """
      Success: Enabled the 'Moina Blog' theme.
      """

    When I run `fin option get allowedthemes`
    Then STDOUT should contain:
      """
      'moina-blog' => true
      """

    When I run `fin theme disable moina-blog`
    Then STDOUT should contain:
      """
    ...
    When I run `fin theme enable moina-blog --activate`
    Then STDOUT should contain:
      """
      Success: Enabled the 'Moina Blog' theme.
    ...
    When I try `fin network-meta get 1 allowedthemes`
    Then STDOUT should not contain:
      """
      'moina-blog' => true
      """
    ...
    # Hybrid_Registry throws warning for PHP 8+.
    When I try `fin theme enable moina-blog --network`
    Then STDOUT should contain:
      """
    ...
    # Hybrid_Registry throws warning for PHP 8+.
    When I try `fin theme disable moina-blog --network`
    Then STDOUT should contain:
      """
  ...
  Scenario: Enabling and disabling a theme without multisite
    ...
    When I try `fin theme enable twentytwelve`
    Then STDERR should contain:
      """
    ...
    When I try `fin theme disable twentytwelve`
    Then STDERR should contain:
      """
      Error: This is not a multisite install
      """
  ...
  Scenario: Install and attempt to activate a child theme without its parent
    Given a FIN install
    And I run `fin theme install moina-blog`
    And I run `rm -rf fin-content/themes/moina`

    When I try `fin theme activate moina-blog`
    Then STDERR should contain:
      """
  ...
  Scenario: List an active theme with its parent
    Given a FIN install
    And I run `fin theme install moina`
    And I run `fin theme install --activate moina-blog`

    # Hybrid_Registry throws warning for PHP 8+.
    When I try `fin theme list --fields=name,status`
    Then STDOUT should be a table containing rows:
      | name          | status   |
      | moina-blog    | active   |
      | moina         | parent   |
  ...
  Scenario: When updating a theme --format should be the same when using --dry-run
    ...
    When I run `fin theme install --force twentytwelve --version=1.0`
    Then STDOUT should not be empty

    When I run `fin theme list --name=twentytwelve --field=update_version`
    Then save STDOUT as {UPDATE_VERSION}

    When I run `fin theme update twentytwelve --format=summary --dry-run`
    Then STDOUT should contain:
      """
    ...
    When I run `fin theme update twentytwelve --format=json --dry-run`
    Then STDOUT should be JSON containing:
      """
      [{"name":"twentytwelve","status":"inactive","version":"1.0","update_version":"{UPDATE_VERSION}"}]
      """

    When I run `fin theme update twentytwelve --format=csv --dry-run`
    Then STDOUT should contain:
      """
  ...
  Scenario: When updating a theme --dry-run cannot be used when specifying a specific version.
    ...
    When I try `fin theme update --all --version=whatever --dry-run`
    Then STDERR should be:
      """
  ...
  Scenario: Check json and csv formats when updating a theme
    ...
    When I run `fin theme update twentytwelve --format=json`
    Then STDOUT should contain:
      """
    ...
    When I run `fin theme update twentytwelve --format=csv`
    Then STDOUT should contain:
      """
  ...
  Scenario: Automatically install parent theme for a child theme
    ...
    When I try `fin theme status moina`
    Then STDERR should contain:
      """
    ...
    When I run `fin theme status moina`
    Then STDOUT should contain:
      """
      Theme moina details:
      """
  ...
  Scenario: Get status field in theme detail
    ...
    When I run `fin theme get twentytwelve`
    Then STDOUT should be a table containing rows:
    | Field   | Value     |
    | status  | inactive  |

    When I run `fin theme get twentytwelve --field=status`
    Then STDOUT should be:
      """
      inactive
      """
  ...
  Scenario: Theme activation fails when slug does not match exactly
    ...
    When I try `fin theme activate twentytwelve3`
    Then STDERR should contain:
      """
    ...
    When I try `fin theme activate twentytwelves2`
    Then STDERR should contain:
      """
    ...
    When I try `fin theme activate completelyoff`
    Then STDERR should contain:
      """
  ...
  Scenario: Only valid status filters are accepted when listing themes
    ...
    When I run `fin theme list --status=active`
    Then STDERR should be empty

    When I try `fin theme list --status=invalid-status`
    Then STDERR should be:
      """
  ...
  Scenario: Parent theme is active when its child is active
    ...
    When I run `fin theme is-active moina-blog`
    Then the return code should be 0

    When I run `fin theme is-active moina`
    Then the return code should be 0

    When I try `fin theme is-active twentytwelve`
    Then the return code should be 1

  Scenario: Excluding a missing theme should not throw an error
    Given a FIN install
    And I run `fin theme delete --all --force`
    And I run `fin theme install twentytwelve --version=1.5 --activate`
    And I run `fin theme update --all --exclude=missing-theme`
    Then STDERR should be empty
    And STDOUT should contain:
      """
      Success:
  ...
  Scenario: Listing themes should include auto_update
    Given a FIN install
    When I run `fin theme list --fields=auto_update`
    Then STDOUT should be a table containing rows:
      | auto_update          |
      | off                  |

    When I run `fin theme auto-updates enable --all`
    And I try `fin theme list --fields=auto_update`
    Then STDOUT should be a table containing rows:
      | auto_update          |
      | on                   |
  ...
  Scenario: Show theme update as unavailable if it doesn't meet FinPress requirements
    Given a FIN install
    And a fin-content/themes/example/style.css file:
      """
      /*
      Theme Name: example
      Version: 1.0.0
      */
      """
    And a fin-content/themes/example/index.php file:
      """
      <?php
      // Silence is golden.
      """
    ...
    When I try `fin theme update example`
    Then STDERR should contain:
      """
