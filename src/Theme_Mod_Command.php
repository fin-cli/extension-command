 ...
 */
class Theme_Mod_Command extends FIN_CLI_Command {
	...
	public function get( $args, $assoc_args ) {
		...
		if ( ! \FIN_CLI\Utils\get_flag_value( $assoc_args, 'all' ) && empty( $args ) ) {
			FIN_CLI::error( 'You must specify at least one mod or use --all.' );
		}

		if ( \FIN_CLI\Utils\get_flag_value( $assoc_args, 'all' ) ) {
			$args = array();
		}
		...
		$formatter = new \FIN_CLI\Formatter( $assoc_args, $this->fields, 'thememods' );
	...
	public function remove( $args, $assoc_args ) {
		...
		if ( \FIN_CLI\Utils\get_flag_value( $assoc_args, 'all' ) ) {
			remove_theme_mods();
			FIN_CLI::success( 'Theme mods removed.' );
			return;
		}
		...
		FIN_CLI::success( sprintf( $success_message, $count ) );
	}
	...
	public function set( $args, $assoc_args ) {
		...
		if ( get_theme_mod( $mod ) === $value ) {
			FIN_CLI::success( "Theme mod {$mod} set to {$value}." );
		} else {
			FIN_CLI::success( "Could not update theme mod {$mod}." );
		}
	}
}
